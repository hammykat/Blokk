...
The engine splits the workload across all threads by **assigning ranges** (determined at the **start of the frame** by the main thread)to each thread. But if the engine sees that no objects were added and no fields were updated in the last frame, it'll **reuse the same ranges from the previous frame**. This offers a performance boost.
